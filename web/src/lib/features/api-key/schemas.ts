...
export const checkApiKeySchema = z.object({
	apiKey: z.string().trim().min(30).startsWith("sn_"),
});
...
export const updateApiKeyExpirationFormSchema = z.object({
	expires_at: z.string().optional().default(""),
});
export type UpdateApiKeyExpirationForm = z.infer<typeof updateApiKeyExpirationFormSchema>;
