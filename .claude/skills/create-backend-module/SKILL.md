...
Create domain modules as layers under `internal/`. Do not put domain CRUD in `drivers/` or `pkg/driver/`.
...
| Repo interface | `I` + Entity + `Repository` | `ILLMRepository` |
| DTOs | `CreateXDTO`, `UpdateXDTO`, `FindXsFilterDTO` | |

Templates: `internal/module/llm` (CRUD), `internal/module/api-key` (tests).
...
- UUID PKs; sensitive fields `json:"-"`; JSONB via `util.JSONMap` or `serializer:json`.
...
- Prefer embedding `IRepository[T]`; override Filter/Find or add scoped methods when needed (see `session`, `agent`).
