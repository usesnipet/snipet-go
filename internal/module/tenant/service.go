...
// after user.Service.Init in bootstrap ordering. Returns the bootstrap
// Tenant so callers that also need to stamp TenantID at boot time (e.g.
// client.Service.Init, for the inherit-client flow) don't have to look it
// up separately.
func (s *Service) Init(ctx context.Context, adminEmail string) (*model.Tenant, error) {
	...
	if err := s.memberRepo.Create(ctx, &model.Member{
		UserID:   admin.ID,
		TenantID: tenant.ID,
...
func (s *Service) FindByID(ctx context.Context, id string) (*model.Tenant, error) {
	if _, err := authz.RequireMember(ctx, id); err != nil {
		return nil, err
	}
...
func (s *Service) FindBySlug(ctx context.Context, slug string) (*model.Tenant, error) {
	...
	if _, err := authz.RequireMember(ctx, tenant.ID); err != nil {
		return nil, err
	}
...
func (s *Service) DeleteByID(ctx context.Context, id string) error {
	if _, err := authz.RequireTenantRole(ctx, id, model.RoleAdmin); err != nil {
		return apperr.Forbidden("not allowed to delete this tenant")
	}
...
func (s *Service) Leave(ctx context.Context, tenantID string) error {
	identity, err := authz.RequireMember(ctx, tenantID)
	if err != nil {
		return err
	}
	member, _ := identity.MembershipOf(tenantID)

	if member.Role.Is(model.RoleAdmin) {
