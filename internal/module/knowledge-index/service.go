...
func (s *Service) Filter(ctx context.Context, knowledgeID string, opts *filter.Options[model.KnowledgeIndex]) (*page.Paginated[model.KnowledgeIndex], error) {
	return s.repo.FilterInKnowledge(ctx, knowledgeID, opts)
}

func (s *Service) FilterItems(ctx context.Context, knowledgeID, indexID string, opts *filter.Options[model.IndexedKnowledgeItem]) (*page.Paginated[model.IndexedKnowledgeItem], error) {
...
func (s *Service) Create(ctx context.Context, knowledgeID string, dto CreateKnowledgeIndexDTO) (*model.KnowledgeIndex, error) {
	if _, err := s.knowledgeRepo.FindByID(ctx, knowledgeID); err != nil {
		return nil, err
	}
