package model

import "github.com/usesnipet/snipet/pkg/jsonx"

type AppUser struct {
	ID string `gorm:"type:uuid;primaryKey;default:gen_random_uuid()" json:"id"`

	Name     string        `gorm:"type:varchar(255);not null" json:"name"`
	Picture  *string       `gorm:"type:text" json:"picture"`
	Email    *string       `gorm:"type:text" json:"email"`
	Metadata jsonx.JSONMap `gorm:"type:jsonb;not null" json:"metadata"`

	AppUserToSessions []AppUserToSession `gorm:"foreignKey:AppUserID;references:ID;constraint:OnDelete:CASCADE" json:"app_user_to_sessions"`
	AppToAppUsers     []AppToAppUser     `gorm:"foreignKey:AppUserID;references:ID;constraint:OnDelete:CASCADE" json:"app_to_App_users"`
}

type AppToAppUser struct {
	AppID      string  `gorm:"primaryKey" json:"app_id"`
	AppUserID  string  `gorm:"primaryKey" json:"app_user_id"`
	ExternalID *string `gorm:"type:varchar(255);index" json:"external_id"`

	App     *App     `gorm:"foreignKey:AppID;references:ID;constraint:OnDelete:CASCADE" json:"app"`
	AppUser *AppUser `gorm:"foreignKey:AppUserID;references:ID;constraint:OnDelete:CASCADE" json:"app_user"`
}

type AppUserToSession struct {
	AppUserID string `gorm:"primaryKey" json:"user_id"`
	SessionID string `gorm:"primaryKey" json:"session_id"`

	AppUser *AppUser `gorm:"foreignKey:AppUserID;references:ID;constraint:OnDelete:CASCADE" json:"app_user"`
	Session *Session `gorm:"foreignKey:SessionID;references:ID;constraint:OnDelete:CASCADE" json:"session"`
}
